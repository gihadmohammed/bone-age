...
function handleNavigation() {
  ...
  if (width < 995) {
    ...
    useEffect(() => {
      ...
      axios.get('http://localhost:8000/Products/homepage-recom/', {
        headers: {
          Authorization:` Bearer ${token}`
        }})
        .then((response)=>{
      ...
      .catch((error) => {
        console.log("Error fetching products recommendations:", error);
      })
    }, []);
...
`                     {userRole === 'CUSTOMER' ?
      ...
      <Offerscontainer photos={photos}/>
      </div>
      <div className={classes.content}>
      <ProductsContainer products={products} title={"Products"}/>
      </div>
      <div className={classes.content}>
        {SimilarProducts.length > 0 && <ProductsContainer products={SimilarProducts} title={"Recommended for you"}/>}
      </div>
...
export default requireAuth(LandingPageUser);